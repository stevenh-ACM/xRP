...
if (HasEnvironmentVariable("ADVANCED_SITE_DIR")) siteDir = Directory(EnvironmentVariable("ADVANCED_SITE_DIR"));
	...
	.Does(() => 
{
	...
	CopyFiles($"{Paths.ProjectFile.GetDirectory()}/bin/{configuration}/**/*.(dll|pdb|xml)", binDir);
});
	...
	.Does(() => 
{
	...
	var webConfigPath = siteDir+File("Web.config");
	var connectionStringXPath = "configuration/location/system.web/pxdatabase/providers/add/@connectionStringName";
	var connectionStringValue = XmlPeek(webConfigPath, connectionStringXPath);
	Information($"Checking {webConfigPath} for PXSqlDatabaseProvider with configured connectionString");
	if (connectionStringValue != "") {
		Information($"Temporarily clearing connectionString with value {connectionStringValue} in {webConfigPath}");
		XmlPoke(webConfigPath, connectionStringXPath, "");
	}
	...
	if (connectionStringValue != "") {
		Information($"Restoring cleared connectionString value of {connectionStringValue} in {webConfigPath}");
		XmlPoke(webConfigPath, connectionStringXPath, connectionStringValue);
	}
